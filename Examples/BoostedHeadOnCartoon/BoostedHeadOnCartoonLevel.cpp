...
#include "BoostedHeadOnCartoonLevel.hpp"

#include "AlgebraicConstraintsEnforcer.hpp"
#include "BoostedHeadOnCartoonInitialData.hpp"
#include "CCZ4RHS.hpp"
...
BHAmr<BoostedHeadOnCartoonLevel::num_punctures> *BoostedHeadOnCartoonLevel::get_bh_amr_ptr()
{
    return dynamic_cast<BHAmr<num_punctures> *>(get_gr_amr_ptr());
}

PunctureTracker<BoostedHeadOnCartoonLevel::num_punctures> &
BoostedHeadOnCartoonLevel::get_puncture_tracker()
{
    return get_bh_amr_ptr()->get_puncture_tracker();
}

void BoostedHeadOnCartoonLevel::variableSetUp()
{
    BL_PROFILE("BoostedHeadOnCartoonLevel::variableSetUp()");
...
void BoostedHeadOnCartoonLevel::specific_advance()
{
...
void BoostedHeadOnCartoonLevel::initData()
{
    BL_PROFILE("BoostedHeadOnCartoonLevel::initData");
    if (get_gr_amr_ptr()->Verbose() > 0)
    {
        amrex::Print() << "BoostedHeadOnCartoonLevel::initData " << Level() << "\n";
    }
...
//
//#else
//    // Set up the compute class for the BoostedHeadOnCartoon initial data
//    amrex::Real dx = Geom().CellSize(0);
//    BoostedHeadOnCartoonInitialData binary_initial_data(dx);
//    static_assert(std::is_trivially_copyable_v<BoostedHeadOnCartoonInitialData>,
//                  "BoostedHeadOnCartoonInitialData needs to be device copyable");
//
...
//#endif
    ...
    if (get_bh_amr_ptr()->puncture_tracking_enabled() && Level() == 0)
    {
        ...
        // tagging
        BoostedBHInitialDataCartoon::params_t bh1_params(1);
        BoostedBHInitialDataCartoon::params_t bh2_params(2);
//#ifdef USE_TWOPUNCTURES
...
void BoostedHeadOnCartoonLevel::specific_eval_rhs(amrex::MultiFab &a_soln,
                                      amrex::MultiFab &a_rhs,
                                      const amrex::Real /*a_time*/)
{
    BL_PROFILE("BoostedHeadOnCartoonLevel::specific_eval_rhs()");
...
void BoostedHeadOnCartoonLevel::specific_update_ode(amrex::MultiFab &a_soln)
{
...
void BoostedHeadOnCartoonLevel::pre_tag_cells()
{
    amrex::MultiFab &state_new = get_new_data(state_index);
...
void BoostedHeadOnCartoonLevel::tag_cells(amrex::TagBoxArray &a_tag_box_array,
                              amrex::Real a_regrid_threshold)
{
    BL_PROFILE("BoostedHeadOnCartoonLevel::tag_cells()");
    amrex::MultiFab &state_new = get_new_data(state_index);
...
void BoostedHeadOnCartoonLevel::specific_post_init()
{
    BL_PROFILE("BoostedHeadOnCartoonLevel::specific_post_init()");
...
void BoostedHeadOnCartoonLevel::specific_post_restart()
{
    BL_PROFILE("BoostedHeadOnCartoonLevel::specific_post_restart()");
...
void BoostedHeadOnCartoonLevel::specific_post_plotfile(const std::string &a_dir,
                                           std::ostream &a_os)
{
    if (get_bh_amr_ptr()->puncture_tracking_enabled() && Level() == 0)
    {
...
void BoostedHeadOnCartoonLevel::specific_post_checkpoint(const std::string &a_chk_dir,
                                             std::ostream & /*a_os*/)
{
    if (get_bh_amr_ptr()->puncture_tracking_enabled() && Level() == 0)
    {
...
void BoostedHeadOnCartoonLevel::specific_post_timestep()
{
    BL_PROFILE("BoostedHeadOnCartoonLevel::specific_post_timestep");
